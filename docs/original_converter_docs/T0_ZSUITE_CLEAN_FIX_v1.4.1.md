...
- `TOOLCHANGE_CLEAN T1 -> T0`: pełny fix, włącznie z `RESTORE E0` po retrakcji starego T1.
- `TOOLCHANGE_CLEAN T0 -> T1`: zachowane dotychczasowe T1, z `RESTORE E0` po retrakcji starego T0.
- `LAYER_CLEAN T0`: poprawiony T0 clean body bez toolchange switch/retract.
- `START_MACHINE SINGLE` i `START_MACHINE DUAL`: nie przepisywane na toolchange clean; zachowane osobne procedury startowe.
